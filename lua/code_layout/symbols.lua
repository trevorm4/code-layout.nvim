...
local function parse_symbols(symbols, level, parent_path)
	...
	for _, symbol in ipairs(symbols) do
		...
		if kind == "Function" or kind == "Method" then
			icon = "󰊕"
		elseif kind == "Class" or kind == "Struct" then
			icon = "󰌗"
		elseif kind == "Interface" then
			icon = "󰭦"
		elseif kind == "Variable" or kind == "Field" then
			icon = "󰫧"
		...
		if config.options.floating.annotate and parent_path ~= "" then
		...
		if symbol.children and #symbol.children > 0 then
			...
			for _, loc in ipairs(child_locations) do
				table.insert(locations, loc)
			end
			for _, f in ipairs(child_fzf) do
				table.insert(fzf_lines, f)
			end
		end
	end
