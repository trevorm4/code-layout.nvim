local api = vim.api
local float = require("code_layout.layout.float")
local normal = require("code_layout.layout.normal")
local M = {}
...
function M:arg_layout(args)
	...
	for _, item in ipairs(args) do
		...
		elseif item:find("float") then
			layout = "float"
		elseif item:find("dropdown") then
			layout = "dropdown"
		end
	end
	return layout
...
function M:right(opt)
	...
	if not self.right_winid then
		print("[code-layout] no enough room to show floating window")
	end
	self.current = RIGHT
	return self
end
...
function M:dropdown(height)
	...
	self.left_bufnr, self.left_winid = require("code_layout.layout.normal").left(height, nil, nil, nil, true)
	return self
end
...
function M:close()
	for i, id in ipairs({ self.left_winid, self.right_winid }) do
		if id and api.nvim_win_is_valid(id) then
			...
			elseif i == 2 and self.layout ~= "dropdown" then
				api.nvim_win_close(id, true)
			end
		end
	end
